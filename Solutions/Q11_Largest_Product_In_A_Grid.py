...
def fastest(grid: List[List[int]] = GRID, length: int = LENGTH) -> int:
    """ This is the fastest function that finds the largest product in a grid series """
    return full_product_search(grid, length)


def full_product_search(grid: List[List[int]] = GRID, length: int = LENGTH) -> int:
    max_product = 0
    rows = len(grid)
    cols = len(grid[0])

    # up/down
    for i in range(0, rows-length+1):
    ...
    for i in range(0, rows):
        for j in range(0, cols-length+1):
...
if __name__ == "__main__":
    ...
    performance_run(full_product_search, iterations=PERFORMANCE_RUNS)()
